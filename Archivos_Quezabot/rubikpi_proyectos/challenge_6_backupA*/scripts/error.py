...
class ErrorCalculus(Node):
    def __init__(self):
        ...
        self.pub_ED= self.create_publisher(
            Float32, "/error_distance", 10
        )

        self.pub_Etheta = self.create_publisher(
            Float32, "/error_theta", 10
        )
        ...
        self.sub_groundtruth = self.create_subscription(
            Odometry, "/encoder_odometry", self.ground_truth_callback, 10
        )
    ...
    def ground_truth_callback(self, msg):
        ...
        x_r, y_r = self.current_pos.pose.pose.position.x, self.current_pos.pose.pose.position.y
        ...
        curr_yaw = self.euler_from_quaternion(self.current_pos.pose.pose.orientation)
