...
class LineFollowerPID(Node):
    ...
    def __init__(self):
        ...
        self.obs_sub_sim         = self.create_subscription(Bool,    '/obstacle_detected_sim',  self.obstacle_callback_sim,     10)
        self.obstacle_sub_real       = self.create_subscription(Bool,    '/obstacle_detected_real',  self.obstacle_callback_real, 10)
        self.at_intersection = False
        self.obstacle_sim        = False
        self.obstacle_real = False
        ...
        self.give_way_active = False  # bandera: lento hasta siguiente intersección
    ...
    def obstacle_callback_sim(self, msg):
        self.obstacle_sim = bool(msg.data)
        if self.obstacle_sim:
            self.get_logger().info("OBSTÁCULO detectado — robot pausado")
    def obstacle_callback_real(self, msg):
        self.obstacle_real = bool(msg.data)

    def intersection_callback(self, msg):
        if not msg.data:
            return

        # Si give_way activo, la intersección lo cancela → velocidad normal
        if self.give_way_active:
            self.give_way_active = False
            self.action_command  = "none"
            self.action_phase    = "none"
            self.get_logger().info("GIVE WAY cancelado por intersección → PID normal")
            return
    ...
    def yolo_callback(self, msg):
        ...
        if cmd == "turn_right":
            ...
            self.get_logger().info("YOLO: ROADWORK — baja velocidad 8s")
            ...
            self.get_logger().info("YOLO: GIVE WAY — espera 2s luego lento hasta siguiente intersección")
    ...
    def control_loop(self):
        ...
        if self.obstacle_sim or self.obstacle_real:
            self.cmd_pub.publish(Twist())
        ...
        # ── GIVE WAY ACTIVE tiene prioridad sobre semáforo ──
        # Si give_way está en fase active, aplica su factor y sale
        if self.action_command == "give_way" and self.action_phase == "active":
            # Respeta rojo pero ignora amarillo (ya va lento)
            if self.last_valid_color == 3.0:
                self.current_linear  = 0.0
                self.current_angular = 0.0
                estado = "ROJO-STOP"
            else:
                # Velocidad fija baja, no multiplicar sobre PID variable
                self.current_linear  = self.base_speed * 0.45
                self.current_angular = self.current_angular * 0.5
        ...
        if self.action_command != "none" and self.action_phase != "none":
            ...
            # ROADWORK: baja velocidad 8s fijos
            ...
            # GIVE WAY: espera 2s luego pasa a active (manejado arriba)
