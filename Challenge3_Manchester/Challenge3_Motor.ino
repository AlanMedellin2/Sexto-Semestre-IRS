...
{  
  ...
  //pwm_set_point = constrain(msg->data, MSG_MIN_VAL, MSG_MAX_VAL);   //constrain, verifica que el mensaje este en esos valores
  //ledcWrite(PWM_CHNL, (uint32_t) ((pow(2, PWM_RES)-1) * (pwm_set_point  / 1.0))); //si cumple, volvemos ese 1 en mensaje pwm
  target_pwm = constrain(msg->data, MSG_MIN_VAL, MSG_MAX_VAL);
}
