...
class LineDetector(Node):
    ...
    def __init__(self):
        ...
        self.img_pub    = self.create_publisher(Image, '/camera/line_ufast_h', 10)
    ...
    def process_frame(self, frame):
        ...
        trap  = np.array([[
            ((roi_w - top_w) //2, top_y),        #arriba iz
            ((roi_w + top_w)// 2, top_y),        #arriba der
            ((roi_w + bottom_w)// 2, bottom_y),  #abajo der
            ((roi_w - bottom_w)//2, bottom_y ) #abajo izquierda
        ]], dtype=np.int32)
