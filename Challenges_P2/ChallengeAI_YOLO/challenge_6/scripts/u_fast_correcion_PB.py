...
class LineDetector(Node):
    ...
    def __init__(self):
        ...
        self.corregirV_pub = self.create_publisher(String, '/line_recovery', 10)
...
# ---- Morfología -------------------------------------------
        ...
        #------Volver a encontrar línea central----------
        centro = len(self.center_line) >= self.min_points_for_lane
        izquierda = len(self.left_line) >= self.min_points_for_lane
        derecha = len(self.right_line) >= self.min_points_for_lane

        if centro:
            recovery_state = "centro"

        elif izquierda and not derecha:
            recovery_state = "girar_derecha" # línea azul

        elif derecha and not izquierda:
            recovery_state = "girar_izquierda" #línea rija

        elif izquierda and derecha:# lado con más puntos como referencia.
            if len(self.left_line) > len(self.right_line):
                recovery_state = "girar_derecha"
            elif len(self.right_line) > len(self.left_line):
                recovery_state = "girar_izquierda"
            else:
                recovery_state = "buscar_centro"

        else:
            recovery_state = "me_perdi"

        corregirV_msg = String()
        corregirV_msg.data = recovery_state
        self.corregirV_pub.publish(corregirV_msg)
        ...
        cv.putText(debug, f"REC:{recovery_state}", 
           (3, 60),
        ...
        self.get_logger().info(
            ...
            f'Recovery:{recovery_state} | Objetivo:{self.target_lane}'
        )
