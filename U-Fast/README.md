...
Los detectores de carriles han tenido un gran auge en los últimos años, especialmente por su importancia en sistemas de asistencia al conductor, vehículos autónomos y aplicaciones de visión computacional en tiempo real. El paper menciona que existen dos enfoques principales para resolver este problema: los métodos tradicionales de procesamiento de imágenes y los métodos basados en aprendizaje profundo, especialmente aquellos que utilizan segmentación semántica.

Los métodos tradicionales suelen apoyarse en características visuales como color, bordes, contraste o transformadas geométricas. Por otro lado, los métodos de segmentación profunda han ganado mayor relevancia porque tienen una mejor capacidad para aprender representaciones complejas de la escena. Sin embargo, estos modelos suelen tener un costo computacional alto, ya que normalmente clasifican cada pixel de la imagen como carril o fondo.
...
Además de la velocidad, también se debe considerar el problema conocido como **no-visual-clue**. Este ocurre cuando no existen pistas visuales claras del carril, por ejemplo, cuando la línea está tapada por un vehículo, afectada por sombras, iluminación extrema, desgaste o condiciones de baja visibilidad. En estos casos, el modelo no puede depender únicamente de la información local de los pixeles, sino que necesita utilizar el contexto completo de la imagen.

U-Fast compite con otros modelos como SAD, que significa **Self Attention Distillation**. SAD busca mejorar la velocidad mediante destilación de atención, pero sigue estando basado en una formulación de segmentación. Por esa razón, aunque es más eficiente que otros métodos previos, todavía puede mantener un costo computacional considerable en comparación con U-Fast.

Otro punto importante es que muchos modelos basados en segmentación representan los carriles como mapas binarios de pixeles, pero no aprovechan directamente propiedades estructurales de los carriles, como su continuidad, suavidad y forma. En la práctica, un carril no debería aparecer de forma irregular ni cambiar bruscamente de posición entre una fila y otra. U-Fast toma esto en cuenta mediante una formulación basada en filas y una pérdida estructural.
